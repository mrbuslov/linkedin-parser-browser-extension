...
describe('extractMutualsList — real LinkedIn fixture (common-connections-search.html)', () => {
  it('parses 6+ profiles with non-empty names from the real /search/ HTML', () => {
    const fs = require('fs');
    ...
    const list = extractMutualsList(main, normalizeProfileUrl);
    expect(list.length).toBeGreaterThanOrEqual(6);
    for (const item of list) {
      expect(item.name).toBeTruthy();
      expect(item.profileUrl).toMatch(/^https:\/\/www\.linkedin\.com\/in\//);
      // Names should NOT contain the degree marker
      expect(item.name).not.toMatch(/•\s*1st|2nd|3rd/);
    }
  });
});
